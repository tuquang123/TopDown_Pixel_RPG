    ...
    public void Consume(ItemData consumable)
{
    if (consumable.restoresHealth)
    {
        ...
        StartCoroutine(HealOverTime(amount, 10f)); // hồi trong 10s
    }
    ...
    if (consumable.restoresMana)
    {
        ...
        StartCoroutine(RestoreManaOverTime(amount, 10f)); // hồi trong 10s
    }
}
...
{
    int healed = 0;
    int ticks = 20; // số lần tick (20 tick trong duration)
    float tickInterval = duration / ticks;
    int amountPerTick = Mathf.Max(1, totalAmount / ticks);
...
private IEnumerator RestoreManaOverTime(int totalAmount, float duration)
{
    int restored = 0;
    int ticks = 20;
    float tickInterval = duration / ticks;
    ...
    for (int i = 0; i < ticks; i++)
    {
        ...
        currentMana = Mathf.Clamp(currentMana + amountPerTick, 0, (int)maxMana.Value);
        int after = currentMana;
        int delta = after - before;
